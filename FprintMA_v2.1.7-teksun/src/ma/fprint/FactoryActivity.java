...
public class FactoryActivity extends Activity implements OnClickListener {
    ...
    public void onClick(View arg0) {
        if (arg0.getId() == R.id.ma_factory_back) {
            finish();
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_INIT_FINISH: // 出厂设置完成
                    String text = getString(R.string.ma_factory_finish);
                    Toast mToast = Toast.makeText(FactoryActivity.this, text, Toast.LENGTH_SHORT);
                    mToast.show();
    ...
    Handler initFactoryHandler = new Handler() {
        public void handleMessage(Message msg) {
            if (msg.what == 1 && nProgressValue < PROGRESS_INIT_FINISH) {
                ...
                textProgress.setText(strProgressPrefix + nProgressValue);
                //Util.print("Factory initFactoryHandler: MSG_END nProgress=" + nProgressValue
    ...
    class initFactoryRunnable implements Runnable {
        @Override
        public void run() {
            Util.print("initFactoryRunnable begin to run");
            int nCount = 0;
            while (true) {
                try {
                    if (bTimeCountStop || nProgressValue >= PROGRESS_INIT_FINISH) {
                        Util.print("Factory initFactoryRunnable: break");
                        break;
                    }
                    Util.print("Factory initFactoryRunnable(1) index=" + nCount++);
                    nProgressValue++;
                    Thread.sleep(50);
                    Util.print("Factory initFactoryRunnable(2) index=" + nCount);
            ...
            Util.print("Factory initFactoryRunnable finish");
        }
    }
